...
mice_vars <- c(get_included_vars(), "died")
...
mi_df_test <- complete(mice_train_test, action = "long", include = FALSE) %>%
  as_tibble() %>%
  mutate(set = rep(train_test_data$set, mice_train_test$m)) %>%
  filter(set == "test")
...
## Calibration curves
```{r}
plot_calibration_test <- function(data, p, model_names){
  plot_data <- data[, c("died", ".imp", p)] %>%
...
plot_calibration_test(mi_df_test, p = lrm_prob_names, model_names = lrm_names)
```
...
for (i in 1:length(glmnet_cvfits)){
...
plot_calibration_test(mi_df_test,
