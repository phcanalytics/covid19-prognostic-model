...
filter_ie <- function(data){
  ...
  message(n_dropped, " (", percent_dropped, "%)",
          " of patients were dropped due to the 2-week cutoff.")
  return(data2)
}
...
train_data <- train_data %>% 
  ...
  ## Convert omorbidities to character
  mutate_at(
...
plot_age_scatter_continuous <- function(vars){
  train_data[, c("age", vars)] %>%
...
plot_age_scatter_continuous(vital_continuous_vars$var)
```

### Age and labs
```{r, message = FALSE, cache = CACHE, fig.height = 8, fig.width = 8}
plot_age_scatter_continuous(lab_vars_t$var) +
...
comorbidities_to_include <- comorbidity_vars %>%
  filter(!var %in% c("diabunc", "diabwc", "hypunc", "hypc", "cci")) %>%
  pull(var) %>%
  c("diab", "hyp")
...
list(lrm_val_age, lrm_val_c, lrm_val_d, lrm_val_dc, lrm_val_all) %>%
  set_names(lrm_names) %>%
  summarize_performance() 
```

## Calibration curves
```{r calCurve, cache = CACHE, fig.width = 8, fig.height = 6}
